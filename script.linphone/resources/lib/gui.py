...
__cwd__ = '/home/'+SYSTEM_USER+'/.kodi/addons/script.linphone'
...
if os.path.exists('/usr/local/bin/linphonecsh'):
    LINPHONE_PATH = '/usr/local/bin/linphonecsh'
elif os.path.exists('/usr/bin/linphonecsh'):
    LINPHONE_PATH = '/usr/bin/linphonecsh'
...
class GUI( xbmcgui.WindowXML ):
    ...
    def define_controls(self):
        ...
        self.btn_bright_minus = self.getControl(self.control_btn_bright_minus_id)
...
def linphonecsh(cmd):
    global LINPHONE_PATH
    xlog(cmd)
    return os.system(LINPHONE_PATH + " " + cmd)
...
def limitLinphonecCodecs():
    ...
    lines = sub.Popen(LINPHONE_PATH + " generic 'codec list'", shell=True, stdout=sub.PIPE).stdout.readlines()
    cindex = 0
    for line in lines:
        dis = 1
...
def linphoneCall(target):
    ...
    CONTACT_NAME = target
    CALLIDS = {}
    linphoneGeneric("call " + CONTACT_NAME)
    CALL_STATUS = 'calling'
...
def initLinphone():
    ...
    if debugging == "true":
        cmd = 'bash -c "%s init -V -d %s %s -l \'/home/%s/linphone.log\' -c \'%s\'"' % (LINPHONE_PATH, debug_level, auto_answer, SYSTEM_USER, LINPHONERC)
    else:
        # need to enable debugger anyway, because status watcher uses linphone log.
        cmd = 'bash -c "%s init -V -d %s %s -l \'/home/%s/linphone.log\' -c \'%s\'"' % (LINPHONE_PATH, 2, auto_answer, SYSTEM_USER, LINPHONERC)
        # cmd = 'bash -c "%s init -V %s -c \'%s\'"' % (auto_answer, LINPHONERC)
    ...
    if auto_register == "true":
        linphoneGeneric("register --host '%s' --username '%s' --password '%s'" % (host, str(user), str(password)))
    ...
    setBrightness(0, brightness)

    xlog("SIP_PROTOCOL = %s SIP_PORT = %s DIRECT_CONNECTION = %s" % (sip_protocol, sip_port, direct_connection))

    status_register = sub.Popen(LINPHONE_PATH + " generic 'status register'", shell=True, stdout=sub.PIPE).stdout.readline()
    linphone_friend_list = sub.Popen(LINPHONE_PATH + " generic 'friend list'", shell=True, stdout=sub.PIPE).stdout.read()
